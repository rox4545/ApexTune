...
function ActionWidget({ icon, title, subtitle, gradient, data, theme }: any) {
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'16px' }}>
        <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize:'11px', color:mutedColor, marginTop:'1px' }}>{subtitle}</div>
          </div>
        </div>
...
export default function BatteryReportWidget() {
  const theme = useTheme();
  const { getToolOutput } = useWidgetSDK();
  const data = getToolOutput<D>();
  return <ActionWidget icon="🔋" title="Battery Report" subtitle="Health & capacity analysis" gradient="linear-gradient(135deg,#22c55e,#16a34a)" data={data} theme={theme} />;
}
