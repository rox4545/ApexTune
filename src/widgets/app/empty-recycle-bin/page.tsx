...
export default function EmptyRecycleBinWidget() {
  const theme = useTheme();
  const { getToolOutput } = useWidgetSDK();
  const data = getToolOutput<D>();
  return <ActionCard icon="♻️" title="Empty Recycle Bin" subtitle="Permanently remove deleted files" gradient="linear-gradient(135deg,#ef4444,#b91c1c)" data={data} theme={theme} />;
}
