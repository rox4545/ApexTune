...
export class NetworkTools {
  @Tool({
    name: 'check_network',
    description: 'Shows active network adapters, IP addresses, and current upload/download throughput.',
  ...
  @Tool({
    name: 'check_latency',
    description: 'Pings a host (default 8.8.8.8) to check network latency and packet loss — a quick way to tell if lag is a network problem.',
    inputSchema: z.object({
  ...
  @Tool({
    name: 'flush_dns',
    description: 'Flushes the local DNS resolver cache — fixes sites that "won\'t load" after a DNS change or bad cache entry. Requires confirm:true — call once first to preview.',
    inputSchema: z.object({
